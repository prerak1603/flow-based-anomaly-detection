...
Module      : 08_train_ensemble_v2_augmented.py
Description : Retrain stacking ensemble on SMOTE + WGAN-GP augmented data
Author      : Prerak Nain
Version     : 2.1.0
...
PURPOSE
--------------------------------------------------------------------------------
Same architecture as the original ensemble (04_train_ensemble.py), but
trained on the augmented dataset (SMOTE + WGAN-GP boosted Heartbleed and
Infiltration). Saved as a SEPARATE model version for direct before/after
...
class Config:
    ...
    MODELS_DIR   = PROJECT_ROOT / "v2" / "models_augmented"
    
    # INPUT: the GAN-augmented training data
    TRAIN_FILE   = RESULTS_DIR / "train_data_gan_augmented.parquet"
    
    # OUTPUT: new model files, separate from original (for comparison)
    ENSEMBLE_FILE = MODELS_DIR / "aegis_v2_1_ensemble_augmented.pkl"
...
def setup_logger():
    logger = logging.getLogger("AegisAI.TrainAugmented")
...
# ==============================================================================
# PIPELINE (same structure as original training script)
# ==============================================================================

def load_training_data():
    print_subheader("STAGE 1/7: LOADING AUGMENTED TRAINING DATA")
    
    df = pd.read_parquet(Config.TRAIN_FILE, engine='fastparquet')
    y = df[Config.LABEL_COLUMN]
    X = df.drop(columns=[Config.LABEL_COLUMN])
    
    print(f"\n  Records: {len(X):,}  |  Features: {X.shape[1]}  |  Classes: {y.nunique()}")
    print(f"\n  Rare class counts:")
    for cls in ["Heartbleed", "Infiltration"]:
        print(f"    {cls:<15} {(y == cls).sum():,}")
...
def preprocess_data(X, y):
    ...
    print(f"\n  Scaled shape: {X_scaled.shape}  |  Classes encoded: {len(label_encoder.classes_)}")
...
def train_meta_learner(X, y, rf_model, xgb_model, lgb_model):
    ...
    log.info("  [1/3] Random Forest out-of-fold predictions...")
    ...
    log.info("  [3/3] LightGBM out-of-fold predictions...")
    ...
    elapsed = (datetime.now() - start).total_seconds()
    acc = meta_model.score(meta_features, y)
    print(f"\n  Time: {format_time(elapsed)}  |  Meta-learner accuracy: {acc:.4f}")
    return meta_model


def save_all_models(rf_model, xgb_model, lgb_model, meta_model, scaler, label_encoder):
    print_subheader("STAGE 7/7: SAVING AUGMENTED MODELS")
    ...
    ensemble = {
        ...
        'version': '2.1.0-augmented',
        'trained_at': datetime.now().isoformat(),
        'augmentation': 'SMOTE + WGAN-GP on Heartbleed/Infiltration'
    }
    joblib.dump(ensemble, Config.ENSEMBLE_FILE)
    
    size_mb = Config.ENSEMBLE_FILE.stat().st_size / (1024*1024)
    print(f"\n  Saved: {Config.ENSEMBLE_FILE.name} ({size_mb:.2f} MB)")


def main():
    start_time = datetime.now()
    print_header("AEGIS AI v2.1 - TRAINING ON AUGMENTED DATA")
    ...
    try:
        ...
        print(f" Next: Evaluate on SAME held-out test set, compare vs original")
